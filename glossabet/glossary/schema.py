"""Bounded validation of an untrusted glossary document.

``validate_glossary`` accepts only schema 1 as ``glossary.model`` declares
...
def _scope_from_raw(
    ...
    for index, prefix in enumerate(prefixes):
        ...
        if prefix is None:
            continue
        if not is_literal_path_prefix(prefix):
            errors.add(
                f"{path_where} must be a literal repository-relative path prefix"
            )
            continue
        valid.append(prefix)
    scope, duplicated, overlapping = normalize_scope(valid)
    if duplicated:
...
_VocabularyClaim = Callable[
    [str, VocabularyOwner, ConceptScope], "VocabularyOwner | None"
]
...
def validate_glossary(glossary: object) -> list[str]:
    ...
    vocabulary_owners: dict[str, ScopeOwnerIndex] = {}

    def claim_vocabulary(
        folded: str, owner: VocabularyOwner, scope: ConceptScope,
    ) -> VocabularyOwner | None:
        index = vocabulary_owners.setdefault(folded, ScopeOwnerIndex())
