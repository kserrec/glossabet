"""Unicode combining-mark ranges (categories Mn, Mc, Me), Unicode 15.0.0.

Python's ``re`` ``\\w`` excludes combining marks, yet they are legal
identifier-continue characters (``("a" + "\u0947").isidentifier()`` is
...
where the interpreter's Unicode version matches.
"""

UNICODE_VERSION = "15.0.0"

MARK_RANGES: tuple[tuple[int, int], ...] = (
...
def mark_class_body() -> str:
    """The ranges as the inside of a ``re`` character class."""
    return "".join(
        f"\\U{start:08x}-\\U{end:08x}" if start != end else f"\\U{start:08x}"
