...
def expected_packet(
    ...
    engine_errors = verify_engine_results(evaluation_path, manifest_path)
    if engine_errors:
        raise EvaluationError(
...
def build_review_results(
    ...
    *,
    ...
    primary = _primary_labels(manifest)
    comparisons = []
    disagreements = []
    secondary_useful = 0
    agreement_count = 0
...
def verify_results(
    ...
    if packet != expected:
        errors.append("reviewer packet is stale or contains non-blinded fields")
    ...
    if (
        ...
        or reviewer.get("input_identity") != _reviewer_input_identity()
        or execution != {
            "ephemeral": True,
            "sandbox": "read-only",
            "approval_policy": "never",
